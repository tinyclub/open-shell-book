...
* `income=$1`：把用户传入的参数赋值给income变量，并在后面作为`awk`的参数，即需要处理的文件
...
说明：很明显，采用第一种办法效率要高很多，因为它提前找出了需要统计的单词，然后再统计，而后者则不然。实际上，如果使用`grep`的`-E`选项，我们无须引入循环，而用一条命令就可以搞定：
